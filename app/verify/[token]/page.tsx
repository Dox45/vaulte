...
export default function VerificationPage() {
  ...
  const [error, setError] = useState<string | null>(null);
  const [logs, setLogs] = useState<{ level: "info" | "success" | "error" | "warn"; msg: string; time: string }[]>([]);
  const [logsOpen, setLogsOpen] = useState(false); // mobile log drawer toggle
  ...
  return (
    <main className="theme-professional min-h-screen bg-[#f0f4f8] text-slate-900 selection:bg-indigo-100 font-sans">
      {/* Background Orbs for Glassmorphism — unchanged */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-indigo-200/40 rounded-full blur-[120px]"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-blue-200/40 rounded-full blur-[120px]"></div>
      </div>

      {/* Header — added responsive horizontal padding; hid encryption status on small screens */}
      <nav className="h-20 border-b border-indigo-100/50 flex items-center px-4 sm:px-8 lg:px-16 justify-between backdrop-blur-md sticky top-0 z-50 bg-white/30">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-indigo-600 flex items-center justify-center rounded-xl shadow-lg shadow-indigo-200">
            <span className="text-white font-black text-xl">V</span>
          </div>
        ...
        <div className="flex items-center gap-4">
          {/* Hidden on mobile — would overlap brand on small screens */}
          <div className="hidden md:flex flex-col items-end">
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">End-to-End Encrypted</div>
            <div className="text-[10px] text-emerald-500 font-bold flex items-center gap-1">
              <span className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse"></span>
          ...
          {/* Mobile-only: logs drawer button — same glassmorphism feel as the rest of the page */}
          <button
            onClick={() => setLogsOpen(v => !v)}
            className="lg:hidden relative px-3 py-1.5 bg-white/50 backdrop-blur-md border border-white/60 rounded-full text-[11px] font-bold uppercase tracking-wider text-slate-500 shadow-sm"
          >
            Logs
            {logs.length > 0 && (
      ...
      {/*
        Mobile log drawer — only rendered on <lg screens when logsOpen=true.
        Visually identical to the desktop sidebar panel (same bg, backdrop, border,
        rounding, typography). Slides up from the bottom as a bottom sheet.
      */}
      {logsOpen && (
        <div className="lg:hidden fixed inset-0 z-40" style={{ top: "80px" }}>
          {/* Tap-outside-to-close backdrop */}
          <div className="absolute inset-0 bg-black/10" onClick={() => setLogsOpen(false)} />

          {/* Panel — identical styling to the desktop sidebar log box */}
          <div className="absolute bottom-0 left-0 right-0 max-h-[60vh] bg-white/40 backdrop-blur-xl border-t border-white/50 rounded-t-[2rem] shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-5 px-8 pb-8 custom-scrollbar">
              {logs.map((log, i) => (
                <div key={i}>
                  <div className="flex justify-between items-center mb-1.5">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${log.level === "error" ? "text-rose-500" :
                        log.level === "success" ? "text-emerald-500" :
                          "text-indigo-500"
                      }`}>{log.level}</span>
                    <span className="text-[10px] font-mono text-slate-400">{log.time}</span>
              ...
              {logs.length === 0 && (
                <div className="flex items-center justify-center py-12 text-slate-300 italic text-sm">
                  System idle...
                </div>
              )}
            </div>
      ...
      {/*
        Layout grid:
        - Mobile/tablet: single column, reduced padding
        - Desktop (lg+): original two-column layout with sidebar
      */}
      <div className="max-w-7xl mx-auto grid lg:grid-cols-[1fr_380px] gap-6 lg:gap-12 p-4 sm:p-8 lg:p-16 relative z-10">

        {/* Main verification card */}
        <section className="flex flex-col">
          <div className="bg-white/60 backdrop-blur-2xl border border-white/50 rounded-2xl sm:rounded-[2rem] p-6 sm:p-10 shadow-2xl shadow-indigo-100/50 relative overflow-hidden">

            {/* Step indicator — allow horizontal scroll on very small screens */}
            <div className="flex items-center gap-3 mb-8 sm:mb-10 overflow-x-auto pb-2 no-scrollbar">
              {[
                { id: "identity", label: "Identity", active: step === "identity" },
                { id: "liveness", label: "Liveness", active: step === "liveness" },
                { id: "voice", label: "Voice", active: step === "voice" },
                ...
                <div key={s.id} className="flex items-center gap-3 shrink-0">
                  <div className={`px-5 py-2.5 rounded-full text-xs font-bold transition-all ${s.active
                      ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200"
                      : "bg-slate-100 text-slate-400"
                    }`}>
                    {idx + 1}. {s.label}
